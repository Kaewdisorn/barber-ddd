...
> `APP_FILTER`, `APP_INTERCEPTOR`, and `APP_PIPE` are all owned by `SharedModule` — `AppModule` stays thin.
...
> `bufferLogs: true` holds NestJS bootstrap logs in memory until the Winston logger is ready — no messages are lost and all output is structured JSON.
